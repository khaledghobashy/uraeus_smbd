...
f.TR.config.AF_jcs_rev_1 = lambda t : np.sin(t)
f.TR.config.AF_jcs_rev_2 = lambda t : np.sin(2*t)
f.TR.config.AF_jcs_rev_3 = lambda t : np.sin(3*t)

assm = f.numerical_assembly()
assm.set_gen_coordinates(assm.q0)
soln = solver(assm)

time_array = np.linspace(0,2*np.pi,250)
