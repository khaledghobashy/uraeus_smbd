...
class numerical_assembly(object):
    ...
    def __init__(self):
        ...
        self.interface_map = {'SU1.vbr_steer': 'ST.rbr_rocker', 'SU1.vbs_ground': 'ground', 'SU1.vbl_steer': 'ST.rbl_rocker', 'SU1.vbs_chassis': 'ground', 'ST.vbs_chassis': 'ground', 'ST.vbs_ground': 'ground', 'TR.vbr_hub': 'SU1.rbr_hub', 'TR.vbr_upright': 'SU1.rbr_upright', 'TR.vbl_hub': 'SU1.rbl_hub', 'TR.vbs_ground': 'ground', 'TR.vbl_upright': 'SU1.rbl_upright', 'TR.vbs_chassis': 'ground', 'TR.vbs_steer_gear': 'ST.rbr_rocker'}
    ...
    def eval_pos_eq(self):
        ...
        for sub in self.subsystems:
            sub.eval_pos_eq()
        self.pos_eq_blocks = sum([s.pos_blocks for s in self.subsystems],[])
    ...
    def eval_vel_eq(self):
        ...
        for sub in self.subsystems:
            sub.eval_vel_eq()
        self.vel_eq_blocks = sum([s.vel_blocks for s in self.subsystems],[])
    ...
    def eval_acc_eq(self):
        ...
        for sub in self.subsystems:
            sub.eval_acc_eq()
        self.acc_eq_blocks = sum([s.acc_blocks for s in self.subsystems],[])
    ...
    def eval_jac_eq(self):
        ...
        for sub in self.subsystems:
            sub.eval_jac_eq()
        self.jac_eq_blocks = sum([s.jac_blocks for s in self.subsystems],[])
